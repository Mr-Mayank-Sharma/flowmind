...
const ALLOW_PRIVATE_NETWORK_HTTP = process.env.ALLOW_PRIVATE_NETWORK_HTTP === "true"
 ...
 * host does not resolve to any private/loopback/link-local range. When
 * ALLOW_PRIVATE_NETWORK_HTTP=true the range checks are skipped (explicit operator opt-in).
 */
export async function assertPublicHttpUrl(rawUrl: string, allowPrivate: boolean = ALLOW_PRIVATE_NETWORK_HTTP): Promise<URL> {
  let url: URL
  try {
    url = new URL(rawUrl)
  } catch {
...
/**
 * Fetch a URL with SSRF protection. Every redirect hop is re-validated against the
 * public-range allowlist before being followed.
 */
export async function fetchPublic(targetUrl: string, options: SafeFetchOptions = {}): Promise<Response> {
  ...
  for (let redirectCount = 0; ; redirectCount++) {
    ...
    if (res.status >= 300 && res.status < 400) {
      ...
      if (redirectCount >= maxRedirects) throw new BlockedUrlError("Too many redirects")
